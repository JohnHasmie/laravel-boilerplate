@inject('model', '\App\Models\Data\Corps')

@extends('backend.layouts.app')

@section('title', __('Create Corps'))

@section('content')
    <x-forms.post :action="route('admin.data.corps.store')">
        <x-backend.card>
            <x-slot name="header">
                @lang('Create Corps')
            </x-slot>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Corps')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection