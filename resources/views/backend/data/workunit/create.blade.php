@inject('model', '\App\Models\Data\WorkUnit')

@extends('backend.layouts.app')

@section('title', __('Create WorkUnit'))

@section('content')
    <x-forms.post :action="route('admin.data.workunit.store')">
        <x-backend.card>
            <x-slot name="header">
                @lang('Create WorkUnit')
            </x-slot>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create WorkUnit')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection