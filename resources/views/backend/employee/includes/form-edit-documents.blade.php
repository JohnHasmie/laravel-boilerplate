            ...
            @foreach ($typeDocument::all() as $document)
                <div class="row m-2">
                    <div class="card" style="width: 15.5rem;">
                        @if ($employee && count($employee->documents))
                            @php $showImage = false @endphp
                            @foreach ($employee->documents as $_document)
                                @if ($_document->type_document_id == $document->id)
                                    <img 
                                        src="{{ env('APP_URL') . '/storage/' . $_document->file }}" 
                                        alt="{{ $document->label }}" 
                                        class="img-thumbnail rounded preview-document"
                                    >
                                    @php $showImage = true @endphp
                                @elseif ($showImage == false)
                                    <img 
